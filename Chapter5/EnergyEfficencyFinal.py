...
y = energy_efficiency.data.targets['Y1']  # Assuming 'Y1' is the primary target variable
...
y_train_tensor = torch.tensor(y_train.to_numpy(), dtype=torch.float32).view(-1, 1)  # Convert to NumPy and reshape
X_test_tensor = torch.tensor(X_test, dtype=torch.float32)
y_test_tensor = torch.tensor(y_test.to_numpy(), dtype=torch.float32).view(-1, 1)  # Convert to NumPy and reshape



# Function to train and plot for a given wrapper
def train_and_plot(wrapper_class, title, filename, style='langevin'):
    net = wrapper_class(input_dim=X_train.shape[1], output_dim=1, no_units=200, learn_rate=1e-5, init_log_noise=0, weight_decay=0.001)
    num_epochs = 3000
...
#train_and_plot(SGD_Wrapper, "SGD Wrapper", filename='EnergySGD', style='sgd')
train_and_plot(Langevin_Wrapper, "Langevin Wrapper (SGLD)", filename='EnergySGLD', style='langevin')
#train_and_plot(MALA_Wrapper, "MALA Wrapper", filename='EnergyMALA', style='mala')