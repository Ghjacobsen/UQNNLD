...
# Add the categorical "ocean_proximity" feature (simulated here)
np.random.seed(42)
housing_data["ocean_proximity"] = np.random.choice(["NEAR BAY", "INLAND"], size=len(housing_data))

# Convert the "ocean_proximity" column to numerical (0 = INLAND, 1 = NEAR BAY)
housing_data["ocean_proximity"] = housing_data["ocean_proximity"].map({"INLAND": 0, "NEAR BAY": 1})

# Splice the dataset to remove true values >= 500,000
...
features = [
    ...
    "MedInc",
    "AveOccup",
    "ocean_proximity",  # Simulated feature
]
...
def train_and_plot(wrapper_class, title, filename, style='langevin'):
    net = wrapper_class(input_dim=X_train.shape[1], output_dim=1, no_units=200, learn_rate=6e-5, init_log_noise=0, weight_decay=0.001)
    num_epochs = 1000
    mix_epochs, burnin_epochs = 100, 600
    nets = []
...
train_and_plot(SGD_Wrapper, "SGD Wrapper", filename='HousingSGD', style='sgd')
train_and_plot(Langevin_Wrapper, "Langevin Wrapper (SGLD)", filename='HousingSGLD', style='langevin')
train_and_plot(MALA_Wrapper, "MALA Wrapper", filename='HousingMALA', style='mala')